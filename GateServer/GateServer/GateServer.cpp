...
void TestRedisMgr() {
    ...
    assert(RedisMgr::GetInstance()->HGet("bloginfo", "blogwebsite", value) != false);
...
int main()
{
    //TestRedis();
    TestRedisMgr();
    static ConfigMgr gCfgMgr=ConfigMgr::GetIns();
    ...
    try
    {
        ...
        signal.async_wait([&ioc](const boost::system::error_code& error, int signal_number) {
            if (error)return;
            ioc.stop();
            });
